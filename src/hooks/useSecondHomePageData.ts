...
export interface SecondHomePageData {
  ...
  /** Top Games from second-home-sections (admin-configured order, no limit). */
...
/** Map backend second-home section game (full object) to GameCardShape; derive badges from API flags. Exported for TopGamesPage. */
...
export function useSecondHomePageData(): {
  ...
  const getSectionMeta = (json: Record<string, unknown>): SectionMeta => ({
    ...
    subtitle: (json.section_subtitle as string) || undefined,
  ...
  const sectionMetaFromSections = sectionsData
    ? {
        providers: { title: sectionsData.providers?.section_title, subtitle: sectionsData.providers?.section_subtitle, svg: sectionsData.providers?.section_svg },
