...
const AdminPlayers = () => {
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (role === "powerhouse" || role === "super") {
                const matchedMaster = (mastersList as { id: number; username?: string }[]).find(
                  (m) => (m.username ?? "").toLowerCase() === String(row.parent_username ?? "").toLowerCase()
                );
                setEditParentId(matchedMaster?.id ?? "");
              } else {
                setEditParentId("");
              }
              setEditOpen(true);
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={createOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <PasswordInput placeholder="Password" value={createPassword} onChange={(e) => setCreatePassword(e.target.value)} className="md:col-span-2" autoComplete="new-password" />
          </div>
          <DialogFooter>
      ...
      {/* Edit Player */}
      <Dialog open={editOpen} onOpenChange={(open) => { setEditOpen(open); if (!open) { setEditSaving(false); setEditParentId(""); } }}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if ((role === "powerhouse" || role === "super") && editParentId === "") {
                  toast({ title: "Please select a Master.", variant: "destructive" });
                  return;
                }
                setEditSaving(true);
      ...
      <Dialog open={resetPwOpen} onOpenChange={setResetPwOpen}>
        <DialogContent className="max-w-xs">
          ...
          <div className="space-y-3">
            ...
            <PasswordInput placeholder="Confirm New Password" value={newPasswordConfirm} onChange={(e) => setNewPasswordConfirm(e.target.value)} autoComplete="new-password" />
          </div>
          <DialogFooter>
          ...
          try {
            ...
            if (pendingAction === "deposit") {
              ...
              await directWithdraw(
                { user_id: userId, amount: Number(amount) || 0, remarks, reference_id: refId, pin },
                role
              );
              queryClient.invalidateQueries({ queryKey: ["admin-players", role] });
