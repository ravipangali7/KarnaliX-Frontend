...
const PowerhouseComingSoon = () => {
  ...
  const items = (listApi as ComingSoonRow[]).sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  ...
  const openEdit = (row: ComingSoonRow) => {
    ...
    setComingDate(row.coming_date ? String(row.coming_date).slice(0, 10) : "");
  ...
  const handleSave = async () => {
    ...
    try {
      if (imageFile) {
        ...
        if (comingDate.trim()) formData.append("coming_date", comingDate.trim());
        ...
        await createComingSoon({
          ...
          coming_date: comingDate.trim() || undefined,
          is_active: isActive,
          order: items.length,
        });
      }
      queryClient.invalidateQueries({ queryKey: ["admin-coming-soon"] });
      toast({ title: "Coming Soon item created." });
      resetForm();
      setCreateOpen(false);
    } catch (e) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-coming-soon"] });
      toast({ title: "Coming Soon item updated." });
      resetForm();
      setEditOpen(false);
    } catch (e) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-coming-soon"] });
      toast({ title: "Coming Soon item deleted." });
      setDeleteOpen(false);
  ...
  const columns = [
    { header: "Order", accessor: (row: ComingSoonRow & { id: string }) => row.order },
    {
      header: "Image",
  ...
  const formFields = (
    <>
      ...
      <div>
        ...
        <Textarea placeholder="Short description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="resize-none" />
      </div>
      <div>
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <Label htmlFor="cs-is-active" className="text-xs">Active</Label>
      </div>
      <div>
