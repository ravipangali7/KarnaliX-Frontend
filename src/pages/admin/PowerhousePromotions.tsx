  ...
  const handleSave = async () => {
    ...
    try {
      if (imageFile) {
        ...
        if (ctaLabel.trim()) formData.append("cta_label", ctaLabel.trim());
        if (ctaLink.trim()) formData.append("cta_link", ctaLink.trim());
  ...
  const formFields = (
    <>
      ...
      <div>
        ...
        <Input placeholder="e.g. Learn More" value={ctaLabel} onChange={(e) => setCtaLabel(e.target.value)} />
      </div>
      <div>
        <Label className="text-xs">CTA link (optional)</Label>
        <Input placeholder="e.g. /promotions or https://..." value={ctaLink} onChange={(e) => setCtaLink(e.target.value)} />
      </div>
      <div>
