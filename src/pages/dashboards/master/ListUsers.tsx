...
export const ListUsers: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.getMasterUsers();
  ...
  const handleCreate = async () => {
    ...
    try {
      await apiClient.createMasterUser(formData);
  ...
  const handleAction = async () => {
    ...
    try {
      if (actionType === 'suspend') {
        await apiClient.suspendMasterUser(selectedUser.id);
        toast({ title: 'Success', description: 'User suspended successfully' });
      } else {
        await apiClient.activateMasterUser(selectedUser.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <FormModal
        ...
        title="Create User"
        description="Add a new user under your supervision"
      >
