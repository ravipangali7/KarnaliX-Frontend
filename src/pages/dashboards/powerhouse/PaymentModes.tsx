...
export const PaymentModes: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.getPowerhousePaymentModes();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await apiClient.createPowerhousePaymentMode(payload);
  ...
  const handleToggle = async (mode: PaymentMode) => {
    try {
      await apiClient.togglePowerhousePaymentMode(mode.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable data={data} columns={columns} loading={loading} searchKeys={['wallet_holder_name']} searchPlaceholder="Search by name..." />
      <FormModal open={modalOpen} onOpenChange={setModalOpen} title="Add Payment Mode" description="Add a new payment method">
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="e.g., My SBI Account" />
          </div>
          <div className="space-y-2">
          ...
          {formData.type === 'BANK' && (
            <>
              <div className="space-y-2"><Label className="text-gray-300">Bank Name</Label><Input value={formData.bank_name} onChange={(e) => setFormData({ ...formData, bank_name: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="State Bank of India" /></div>
              <div className="space-y-2"><Label className="text-gray-300">Account Number</Label><Input value={formData.account_number} onChange={(e) => setFormData({ ...formData, account_number: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="1234567890" /></div>
              <div className="space-y-2"><Label className="text-gray-300">Account Name</Label><Input value={formData.account_name} onChange={(e) => setFormData({ ...formData, account_name: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="John Doe" /></div>
              <div className="space-y-2"><Label className="text-gray-300">IFSC Code</Label><Input value={formData.ifsc_code} onChange={(e) => setFormData({ ...formData, ifsc_code: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="SBIN0001234" /></div>
            </>
          )}
          {formData.type === 'UPI' && <div className="space-y-2"><Label className="text-gray-300">UPI ID</Label><Input value={formData.upi_id} onChange={(e) => setFormData({ ...formData, upi_id: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="yourname@upi" /></div>}
          {formData.type === 'EWALLET' && <div className="space-y-2"><Label className="text-gray-300">Wallet Phone Number</Label><Input value={formData.wallet_phone} onChange={(e) => setFormData({ ...formData, wallet_phone: e.target.value })} className="bg-gray-700 border-gray-600 text-white" placeholder="9876543210" /></div>}
          <div className="flex justify-end gap-3 pt-4">
            <Button variant="outline" onClick={() => setModalOpen(false)} className="border-gray-600 text-gray-300">Cancel</Button>
            <Button onClick={handleCreate} disabled={actionLoading} className="bg-yellow-600 hover:bg-yellow-700">{actionLoading ? 'Creating...' : 'Add Payment Mode'}</Button>
          </div>
        </div>
      </FormModal>
    </div>
