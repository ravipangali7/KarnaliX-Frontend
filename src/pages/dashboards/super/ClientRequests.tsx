  ...
  const fetchData = async () => {
    ...
    try {
      const response = type === 'deposit' 
        ? await apiClient.getSuperDeposits()
        : await apiClient.getSuperWithdrawals();
  ...
  const handleAction = async () => {
    ...
    try {
      if (actionType === 'approve') {
        await apiClient.approveSuperRequest(selectedRequest.id);
        toast({ title: 'Success', description: 'Request approved successfully' });
      } else {
        await apiClient.rejectSuperRequest(selectedRequest.id, rejectRemarks);
...
export const TotalDW: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await apiClient.getSuperTotalDW();
        setStats(response);
      } catch (error) {
...
// Master D/W Component
export const MasterDW: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiClient.getSuperMasterDW();
        setData(response.results || response || []);
      } catch (error) {
  ...
  const columns: Column<any>[] = [
    { key: 'username', header: 'Master', sortable: true },
    { 
      key: 'total_deposits', 
      header: 'Deposits',
      sortable: true,
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-white">Master D/W Breakdown</h1>
      ...
      <DataTable
        ...
        loading={loading}
        searchKeys={['username']}
        searchPlaceholder="Search by master username..."
      />
    </div>
  );
};
