...
export const PaymentModes: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.getSuperPaymentModes();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await apiClient.createSuperPaymentMode(payload);
  ...
  const handleToggle = async (mode: PaymentMode) => {
    try {
      await apiClient.toggleSuperPaymentMode(mode.id);
      ...
      <FormModal open={modalOpen} onOpenChange={setModalOpen} title="Add Payment Mode" description="Add a new payment method">
        <div className="space-y-4 py-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button onClick={handleCreate} disabled={actionLoading} className="bg-blue-600 hover:bg-blue-700">{actionLoading ? 'Creating...' : 'Add Payment Mode'}</Button>
          </div>
        </div>
      </FormModal>
    </div>
