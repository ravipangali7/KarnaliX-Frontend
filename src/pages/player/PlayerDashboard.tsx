...
const PlayerDashboard = () => {
  ...
  const handleTransferSubmit = async () => {
    ...
    if (!username || !transferAmount || !transferPassword) {
      toast({ title: "Please fill recipient, amount, and password.", variant: "destructive" });
      return;
    }
    ...
    <div className="p-2 mobile:p-4 md:p-6 space-y-4 mobile:space-y-5 max-w-5xl mx-auto min-w-0">
      ...
      <Dialog open={transferOpen} onOpenChange={setTransferOpen}>
        <DialogContent className="max-w-[calc(100vw-2rem)] mobile:max-w-sm gaming-card w-full">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setTransferOpen(false)} disabled={transferSubmitting}>Cancel</Button>
